 ...
 */
  ...
  router.get("/", async function (req, res, next) {
    ...
    // Upon user filter query, data returns at strings. String to integers transformation:
    ...
    try{
      const validator = jsonschema.validate(query, companyQuery);
      if (!validator.valid) {
        const errs = validator.errors.map(e => e.stack);
        throw new BadRequestError(errs);
      }
      // Pass the filters to the model for querying
